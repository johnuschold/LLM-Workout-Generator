...
# ----------------------------------
# Cached loaders (lazy; only run when called)
# ----------------------------------
...
@st.cache_resource(show_spinner=False)
def load_vector_stores(path: str = "./faiss_index_exercises"):
    return load_vectorstore(path)

@st.cache_data(show_spinner=False)
def load_exercises_data(csv_path: str = "data/exercises_data_final.csv") -> pd.DataFrame:
    try:
...
# Utilities
# ----------------------------------
def extract_exercise_names(workout_text: str):
    exercise_names = []
...
def get_clean_instructions(combined_text: str):
    if pd.isna(combined_text) or not str(combined_text).strip():
        return "No instructions available."
    instructions_block = str(combined_text).split("\n\n")[0].strip()
    matches = re.findall(r"(\d+)\n(.+?)(?=\n\d+|\Z)", instructions_block, re.DOTALL)

    formatted_instructions = []
    if matches:
        for _, instruction_text in matches:
        ...
        if len(lines) >= 2 and lines[0].strip().isdigit():
            ...
            return f"* {instructions_block}" if instructions_block else instructions_block
...
with st.expander("Workout Plan Inputs", expanded=True):
    user_goals = st.selectbox(
        "What are your primary fitness goals?", ["muscle gain", "weight loss", "health"], index=0
    )

    fitness_level = st.selectbox(
        "What is your current fitness level?", ["beginner", "intermediate", "advanced"], index=1
    )

    selected_split_name = st.selectbox(
        "Choose a workout split that fits your preferences:", list(WORKOUT_SPLITS.keys())
    )
    ...
    if selected_split["adjustable"] and len(selected_split["allowed_days"]) > 1:
        workout_days = st.radio(
            ...
            index=selected_split["allowed_days"].index(selected_split["default_days"])
            if selected_split["default_days"] in selected_split["allowed_days"]
            else 0,
    ...
    equipment = st.multiselect(
        "What equipment do you have available?", options=["All"] + all_equipment, default=["All"]
    )
    if "All" in equipment:
...
if "workout_plan_generated" not in st.session_state:
    ...
    st.session_state.exercises_df = None  # filled after first generation
...
if st.button("Generate Workout Plan", key="generate_workout_tab"):
    if not equipment:
        ...
        # Lazy-load heavy resources only now
        with st.spinner("Warming up AI engine & loading data..."):
            llm_pipeline = load_llm()
            vectorstore_exercise = load_vector_stores("./faiss_index_exercises")
            exercises_df = load_exercises_data()

        if exercises_df.empty:
            st.error(
                "Exercise dataset not found. Make sure 'data/exercises_data_final.csv' is present in the deployed app."
            )
        else:
            with st.spinner("Generating your workout plan..."):
                rag_base_query = special_instructions.strip() if special_instructions else ""
                ...
                rag_retrieved_exercises = retrieve_similar_documents(
                    query=improved_query_for_rag, vectorstore=vectorstore_exercise, k=20
                )
            ...
            st.session_state.exercises_df = exercises_df
...
if st.session_state.workout_plan_generated:
    ...
    exercises_df = st.session_state.exercises_df or load_exercises_data()
    ...
    if extracted_exercises:
        ...
        selected_exercise_name = st.selectbox(
            "Choose an exercise to see details:", options=display_options, key="exercise_dropdown"
        )

        if selected_exercise_name and selected_exercise_name != "Select an exercise...":
            if "Exercise Name" not in exercises_df.columns:
                st.warning("Your exercises CSV is missing the 'Exercise Name' column.")
            else:
                all_db_exercise_names = (
                    exercises_df["Exercise Name"].dropna().astype(str).unique().tolist()
                )

                # Strict token_sort_ratio first
                ...
                # Fallback to partial_ratio with a higher cutoff
                ...
                if best_match:
                    ...
                    matching_rows = exercises_df[
                        exercises_df["Exercise Name"].astype(str) == matched_db_exercise_name
                    ]
                    if not matching_rows.empty:
                        ...
                        instructions = get_clean_instructions(
                            exercise_data.get("Combined_Text", "")
                        )
                        st.markdown(instructions)

                        video_url = str(exercise_data.get("Video_URL", "")).strip()
                        if video_url:
                            st.video(video_url)
                        else:
                        ...
                        st.warning(
                            f"Details for '{matched_db_exercise_name}' were found via fuzzy matching, "
                            "but the exact entry could not be retrieved. Please check your data."
                        )
                else:
                    st.warning(
                        f"Details for '{selected_exercise_name}' not found in the exercises database even with fuzzy matching."
                    )

st.markdown("---")
st.info(
    "Disclaimer: This tool provides AI-generated suggestions and should not replace professional advice from a certified fitness trainer. "
    "Always consult with a healthcare professional before making significant changes to your exercise routine."
)
